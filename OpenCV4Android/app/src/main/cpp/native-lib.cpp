...
extern "C" void DetectSkin(int h, int w, uchar** input_frame) {
    ...
    resize(output_frame, output_frame, Size(), 2, 2, INTER_LINEAR);	// pixels are not 0 or 255 only
    ...
    std::copy(out.data, out.data + h*w*3, *input_frame);
}

int main()
{
    /*
    VideoCapture c("sample2.mp4");
    Mat frame;
    clock_t begin;
    while (true) {
        c >> frame;
