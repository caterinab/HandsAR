...
static void
dist_transf_1D_L1NormRet (double *line, uchar *path, int dim, int len)
{
  int q;
...
static void
dist_transform_1D_L1NormRet (double *line, uchar *path, int dim, int len, int step)
{
  int q, ind;
...
void
DistanceTransform2D_NormL1Ret(double *map, int rows, int cols, double *dt, uchar *path)
{
...
bool
DistanceTransform2D_NormL1_GetClosest(int r, int c, uchar *path, int rows, int cols, int *outr, int *outc)
{
  int code, dim, index;
  int r0 = r, c0 = c;
  bool moved = false;
...
static void
dist_transform_1D_L2NormRet(double *line, int len, double *res, int step, int *aux_int, double *aux_double, int *closer, int cstep, int base)
{
  double *DT = res;
  double *L = line;
  int    *v = aux_int;
...
void
DistanceTransform2D_NormL2Ret(double *map, int rows, int cols, double *dt, int *path)
{
  double *tmp = new double[rows*cols];
  int len =  Maximum(rows, cols)+1;
  int *v = new int[len];
...
bool
DistanceTransform2D_NormL2_GetClosest(int r, int c, int *path, int rows, int cols, int *outr, int *outc)
{
