...
extern "C" void DetectSkin(int h, int w, uchar** input_frame, uchar** hands, uchar** cubes) {
	...
	*/
	...
	imshow("", visibleOutput);
	waitKey(0);
	Mat v;
	boxFilter(visibleOutput, visibleOutput, -1, Size(11, 11));
	imshow("", visibleOutput);
	waitKey(0);
	...
	morphologyEx(visibleOutput, visibleOutput, CV_MOP_ERODE, Mat1b(9, 9, 1), Point(-1, -1), 1);
...
{/*
	...
	*/
	...
	//while (true) {
		//c >> frame;
		begin = clock();
		DetectSkin(img.rows, img.cols, &img.data, &img2.data, &img3.data);
		clock_t end = clock();
